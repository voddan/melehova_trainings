...
union PageKey {
    struct {
        // как я понял, нужно CHAR заменить на UINT, чтобы структура была компактна
        // в памяти (без мусора внутри)
        CHAR cColor: 8;
        UINT cAddr: 24;
    };
...
void PageStrgInit() {
    ...
    // как я понял, PAgeStrg - указатель, поэтому нужно поставить *
    memset(PageStrg, 0, sizeof(PageStrg));
}

PageDesc * PageFind(void * ptr, char color) {
    // проверка на адекватность параметра color
